...
if ($modifiedPolicies -eq $null) {
    ...
    foreach ($modifiedPolicy in $modifiedPolicies) {
        ...
        Write-Host "##[debug] Policy $($azurePolicy.displayName)"
        ...
        if ($azurePolicy.name) {
            ...
            $policy.PolicyName = $azurePolicy.displayName
        }

        #set Name variable
        $policy.PolicyDisplayName = $azurePolicy.displayName
        $policy.PolicyDescription = $azurePolicy.description
        $policy.PolicyMode = $azurePolicy.mode
        $policy.PolicyMetadata = $azurePolicy.metadata | ConvertTo-Json -Depth 100
        $policy.PolicyRule = $azurePolicy.policyRule | ConvertTo-Json -Depth 100
        $policy.PolicyParameters = $azurePolicy.parameters | ConvertTo-Json -Depth 100
        $policyList += $policy
    }
